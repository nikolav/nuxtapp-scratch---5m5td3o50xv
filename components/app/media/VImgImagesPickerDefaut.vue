...
const imagesPicked = computed({
  get: () =>
    isEmpty(mImagesPicked_.value)
      ? map(props.defaultImages, (img: string) => ({
          dataurl: img,
          file: { name: null },
        }))
      : mImagesPicked_.value,
  set: (images) => {
    mImagesPicked_.value = images;
  },
});
...
const imageCurrent = computed(() =>
  get(
    imagesPicked.value,
    "0.dataurl",
    first(props.defaultImages) || props.defaultNoImage
  )
);
